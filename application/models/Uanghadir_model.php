...
class Uanghadir_model extends CI_Model
{
    public function read_all_data()
    {
        return $this->db->get_where('vw_uanghadir')->result();
    }
    ...
    public function deleteRpl($bulan, $tahun)
    {
    ...
    public function update_uah($data, $id_payroll_kary)
    {
        $this->db->where('id_payroll_kary', $id_payroll_kary);
        $this->db->where('id_tambahan', 3);
        $this->db->update('tb_payroll_detail', $data);
        return true;
    }

    public function get_uah($id_payroll_kary)
    {
        $query = $this->db->query("SELECT * FROM tb_payroll_detail WHERE id_payroll_kary =" . $id_payroll_kary . " AND id_tambahan=3")->row();
        return $query;
    }
}