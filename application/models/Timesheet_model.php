...
class Timesheet_model extends CI_Model
{
    ...
    public function insert($data, $bln)
    {
        $db_proll = $this->load->database('db_payroll_att', true);
        $bn = intval($bln);
        $insert = $db_proll->insert('tb_b' . $bn, $data);
        if ($insert) {
            return true;
        }
    }
    ...
    {
        $db_proll = $this->load->database('db_payroll_att', true);
        return $db_proll->get_where('tb_kary', ['tgl_hapus' => '1970-01-01 00:00:00'])->result();
    }

    public function get_data_ts_by_bulan($bln, $tahun)
    {
        if ($bln != 0 && $tahun != 0) {
            $db_proll = $this->load->database('db_payroll_att', true);
            $bn = intval($bln);
            return $db_proll->get_where('vw_b' . $bn, ['tahun' => $tahun])->result();
        } else {
            return;
        }
    }

    public function read_data_by_nik($no_nik)
    {
        $db_proll = $this->load->database('db_payroll_att', true);
        return $db_proll->get_where('db_payroll_att.tb_kary', ['no_nik' => $no_nik])->row();
    }

    public function read_data_in_bulan($id_kary, $bln, $tahun)
    {
        $db_proll = $this->load->database('db_payroll_att', true);
        $bn = intval($bln);
        return $db_proll->get_where('tb_b' . $bn, ['id_kary' => $id_kary, 'tahun' => $tahun])->row();
    }
