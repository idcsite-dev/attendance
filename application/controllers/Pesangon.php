...
class Pesangon extends MY_Controller
{
    ...
    {
        $this->load->view('data_master/pesangon/view');
    }

    public function dataTables()
    {
        $source = 'vw_pesangon';
        $data = $this->psg->read_all_data($source);
        $data['inst'] = $data;
        $this->load->view('data_master/pesangon/table', $data);
    }
    ...
    public function import()
    {
        ...
        echo json_encode(array('statusCode' => 200, 'pesan' => 'Import Data Pesangon Berhasil'));
    }
    ...
    public function sample()
    {
        ...
        $sheet->setCellValue('A3', "DATA PESANGON KARYAWAN");
        ...
        $sheet->setCellValue('J5', "NILAI PESANGON");
        ...
        $sheet->setTitle("Sample Import Pesangon");
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="ImportPesangonKaryawan.xlsx"');
    ...
    {
        $pesangon = htmlspecialchars(str_replace(",", "", $this->input->post("pesangon", true)));
        ...
        if (empty($dtkary)) {
            ...
            $dtins = $this->payroll->read_data_detail_psg($id_payroll_kary);

            if (!empty($dtins)) {
                echo json_encode(array('statusCode' => 400, 'pesan' => 'Pesangon gagal ditambahkan!, Data sudah ada'));
            } else {
                $data = array(
                    'id_payroll_kary' => $dtkary->id_payroll_kary,
                    'id_tambahan' => 19,
                ...
                if ($result) {
                    $tdkttp = intval($total_tunj_tidak_tetap) + intval($pesangon);
                    $dttunj = [
                        'total_tunj_tidak_tetap' => $tdkttp,
                    ];

                    $this->payroll->update_lembur($dttunj, $id_payroll_kary);
                    echo json_encode(array('statusCode' => 201, 'pesan' => 'Pesangon berhasil ditambahkan!'));
                } else {
                    echo json_encode(array('statusCode' => 400, 'pesan' => 'Pesangon gagal ditambahkan!'));
                }
            }
    ...
    public function delete()
    {
        ...
        $dtdetins = $this->payroll->read_data_det_psg($id);
        if (!empty($dtdetins)) {
            $nilai = $dtdetins->nilai;
            $id_payroll_kary = $dtdetins->id_payroll_kary;
        } else {
            echo json_encode(array('statusCode' => 400, 'pesan' => 'Pesangon gagal dihapus!'));
            die;
        }
        ...
        if ($result) {
            ...
            echo json_encode(array('statusCode' => 200, 'pesan' => 'Pesangon berhasil dihapus!'));
        } else {
            echo json_encode(array('statusCode' => 400, 'pesan' => 'Pesangon gagal dihapus!'));
        }
    }

    public function deletePsg()
    {

        $bulan = htmlspecialchars($this->input->get('bln'));
        $tahun = htmlspecialchars($this->input->get('thn'));

        $result = $this->psg->deletePsg($bulan, $tahun);
        if ($result) {
            redirect(base_url('pesangon'));
        } else {
            redirect(base_url('pesangon'));
        }
    }
