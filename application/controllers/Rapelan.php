...
class Rapelan extends MY_Controller
{
    ...
    {
        $this->load->view('data_master/rapelan/view');
    }

    public function dataTables()
    {
        $source = 'vw_rapelan';
        $data = $this->rpl->read_all_data($source);
        $data['inst'] = $data;
        $this->load->view('data_master/rapelan/table', $data);
    }
        ...
        foreach ($object->getWorksheetIterator() as $worksheet) {
            ...
            for ($row = 6; $row <= $highestRow; $row++) {
                ...
                if ($query) {
                    ...
                    // bp
                    if ($nilai > 0) {
                        $query = $this->payroll->get_bp($id_payroll_kary);
                        if (!empty($query)) {
                            $bp = $query->nilai;
                            $bpnilai = (intval($nilai) * 3) / 100;
                            $bpnew = intval($bp) + intval($bpnilai);
                            $dtbp = [
                                'nilai' => intval($bpnew),
                            ];
                            $this->payroll->update_bp($dtbp, $id_payroll_kary);
                        }
                    }
                }
    ...
    public function sample()
    {
        ...
        $sheet->setCellValue('A3', "DATA RAPELAN KARYAWAN");
        ...
        $sheet->setCellValue('J5', "NILAI RAPELAN");
        ...
        $sheet->setTitle("Sample Import Rapelan");
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="ImportRapelanKaryawan.xlsx"');
    ...
    public function create()
    {
        ...
        if (empty($dtkary)) {
            ...
            $dtins = $this->payroll->read_data_detail_rpl($id_payroll_kary);
    ...
    public function deleteRpl()
    {
        ...
        $result = $this->rpl->deleteRpl($bulan, $tahun);
        if ($result) {
            redirect(base_url('rapelan'));
        } else {
            redirect(base_url('rapelan'));
        }
    }
