$(document).ready(function () {
  ...
  // Cleave
  // new Cleave("#uanghadir", {
  //   numeral: true,
  //   numeralThousandsGroupStyle: "thousand",
  // });

  // Load DataTables
  $("#dataTable").load(site_url + "Uanghadir/dataTables", function () {
    $("#table1").DataTable();
    initTooltips();
  });
  ...
  // Submit Event
  $("#addUanghadir").submit(function () {
    $.LoadingOverlay("show");
    let uanghadir = $("#uanghadir").val();
    ...
    if (id_karyawan == "") {
      ...
      $.ajax({
        ...
        error: function () {
          $.LoadingOverlay("hide");
          Swal.fire("Server Error", "Gagal menyimpan data Uang Hadir", "error");
        },
      });
    }
  });
  ...
  $("#updateGaji").submit(function () {
    $.LoadingOverlay("show");
    let id = $("#updateId").val();
    let uanghadir = $("#updateGajiPokok").val();
    $.ajax({
      type: "POST",
  ...
  $("#modalImport").submit(function () {
    ...
    if (fileExtension != "xlsx") {
      Swal.fire({
        title: "Informasi",
        text: "File import uanghadir yang dipilih bukan Excel(.xlsx)",
        icon: "info",
      });
    } else if (sizeFile > 500000) {
      Swal.fire({
        title: "Peringatan",
        text: "Ukuran File Import Uanghadir yang dipilih melebihi 500kb",
        icon: "warning",
      });
    } else {
      $.LoadingOverlay("show");
      ...
      $.ajax({
        ...
        error: function () {
          ...
          Swal.fire(
            "Server Error",
            "Gagal mengimport data Uang Hadir",
            "error"
          );
        },
      });
  ...
  // Click Event
  $("#HapusUanghadir").click(function () {
    let bulan = $("#lstBulanHapus").val();
    let tahun = $("#lstTahunHapus").val();

    window.location.href =
      site_url + "uanghadir/deleteRpl?bln=" + bulan + "&thn=" + tahun;
  });

  // Click Event
  $("#exportUanghadir").click(function () {
    let bulan = $("#lstBulan").val();
    let tahun = $("#lstTahun").val();

    window.location.href =
      site_url + "uanghadir/sample?bln=" + bulan + "&thn=" + tahun;
  });
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (data.statusCode == 200) {
          ...
          $("#updateGajiPokok").val(data.uanghadir);
      ...
      error: function () {
        $.LoadingOverlay("hide");
        Swal.fire("Server Error", "Gagal mengambil data uanghadir", "error");
      },
    });
  });
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (data.statusCode == 200) {
          ...
          $("#detailGaji").val(formatCurrency(data.uanghadir));
